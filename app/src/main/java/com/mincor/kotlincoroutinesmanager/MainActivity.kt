...
class MainActivity(private val coroutinesManager: ICoroutinesManager = CoroutinesManager()) :
    ...
    @SuppressLint("SetTextI18n")
    private fun onRepeateButtonClickListener() = launchOnUITryCatchFinally(tryBlock = {
        ...
        val result = asyncWorker.doSomeHardWork()
...
class AsyncWorker(private val asyncTaskManager: IAsyncTasksManager = AsyncTasksManager()) :
    IAsyncTasksManager by asyncTaskManager {

    suspend fun doSomeHardWork() = asyncAwait {
        Thread.sleep(5000)
